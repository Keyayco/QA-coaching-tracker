var Tracker = (function () {
  ...
  var FIELD_NAMES = {
    ...
    articleDepartmentIds: 'Department IDs',
    articleTemplateIds: 'Template IDs',
    articleCriteria: 'Criteria',
    articleTags: 'Tags',
    articleStatus: 'Status',
    articleVersion: 'Version',
    articleAuthor: 'Author',
    articleCreatedDate: 'Created Date',
    articleUpdatedDate: 'Updated Date',
    articlePurpose: 'Purpose',
    articlePolicy: 'Policy',
    articleProcedure: 'Procedure',
    articleExamples: 'Examples',
    articleCommonMistakes: 'Common Mistakes',
    articleCoachingNotes: 'QA Coaching Notes',
    articleRelatedIds: 'Related Article IDs',
    coach: 'Coach',
    coachingAuditId: 'Audit ID',
    coachingAuditDate: 'Audit Date',
    coachingTemplateId: 'Scorecard Template ID',
    coachingType: 'Coaching Type',
    coachingStatus: 'Overall Status',
    priority: 'Priority',
    goal: 'Goal',
    coachingSummary: 'Summary',
    coachingStrengths: 'Strengths',
    coachingDevelopmentAreas: 'Development Areas',
    actionPlan: 'Action Plan',
    successMeasure: 'Success Measure',
    coachingDueDate: 'Due Date',
    reAuditDate: 'Re-audit Date',
  ...
  var COACHING_TYPE_OPTIONS = ['Verbal', 'Written', 'Formal', 'Recognition'];

  var KB_CATEGORIES = ['SOPs', 'Policies', 'Coaching Guides', 'Quick Reference Guides', 'FAQs', 'Best Practices', 'Release Notes'];
  ...
  function buildAuditReport(dataset, filters) {
    ...
    var relatedCoaching = getCoachingSessions({}).sessions.filter(function (s) {
      return matchesAuditId(record, s.auditId);
    });
    ...
    return {
      ...
      coachingExists: relatedCoaching.length > 0,
      relatedCoaching: relatedCoaching,
      performance: {
        score: record.score,
        criticalFailures: criticalFailures,
        failedCriteria: record.failedCriteria,
  ...
  function buildAgentReport(dataset, filters) {
    ...
    var coachingSessions = getCoachingSessions({ agentId: agentId }).sessions;
    ...
    return {
      ...
      coachingExists: coachingSessions.length > 0,
  ...
  // =========================================================================
  // KNOWLEDGE BASE MODULE (Phase A: data model + basic CRUD + homepage)
  // Reuses existing conventions rather than inventing new ones: comma-
  // separated ID lists for Department/Template/Criteria/Tags/Related links
  // (same pattern as Weekly Review Detail's Failed Criteria), the generic
  // updateRowById() helper for edits, and generateNextSequentialId() for IDs.
  // Full versioning (branch-on-edit), the rich text editor, and Criterion
  // Integration (surfacing articles next to failed criteria elsewhere in
  // the app) are later phases - this lays the foundation they'll build on.
  // =========================================================================
  ...
  function mapArticleSummaryRow(row) {
    return {
      articleId: toSafeString(getFieldValue(row, [FIELD_NAMES.articleId])),
      title: toSafeString(getFieldValue(row, [FIELD_NAMES.articleTitle])),
      category: toSafeString(getFieldValue(row, [FIELD_NAMES.articleCategory])),
      status: toSafeString(getFieldValue(row, [FIELD_NAMES.articleStatus])),
      version: toSafeString(getFieldValue(row, [FIELD_NAMES.articleVersion])),
      author: toSafeString(getFieldValue(row, [FIELD_NAMES.articleAuthor])),
      updatedDate: serializeValue(getFieldValue(row, [FIELD_NAMES.articleUpdatedDate])),
      tags: parseCommaList(getFieldValue(row, [FIELD_NAMES.articleTags])),
      departmentIds: parseCommaList(getFieldValue(row, [FIELD_NAMES.articleDepartmentIds])),
      criteria: parseCommaList(getFieldValue(row, [FIELD_NAMES.articleCriteria]))
    };
  }

  // Homepage: one card per fixed category (SOPs, Policies, etc.) with a
  // published-article count and the most recent update within it. This is
  // lightweight on purpose - it never loads article bodies, only the list
  // sheet's summary columns, per the "load lists efficiently, full content
  // only when opened" requirement.
  function getKnowledgeBaseHomepage() {
    var articleData = readSheet(SHEETS.knowledgeBaseArticles);
    var summaries = articleData.rows.map(mapArticleSummaryRow);

    var cards = KB_CATEGORIES.map(function (category) {
      var inCategory = summaries.filter(function (a) {
        return normalizeHeader(a.category) === normalizeHeader(category) && normalizeHeader(a.status) === 'published';
      });
      var lastUpdated = inCategory.reduce(function (latest, a) {
        var date = parseDate(a.updatedDate);
      ...
      return {
        ...
        lastUpdated: lastUpdated ? formatDate(lastUpdated) : ''
      };
    });

    return { success: true, categories: cards };
  }

  // List/search articles (summary fields only - see mapArticleSummaryRow).
  // filters: { category, status, departmentId, searchText }
  function getKnowledgeBaseArticles(filters) {
    var f = filters || {};
    var articleData = readSheet(SHEETS.knowledgeBaseArticles);
    var summaries = articleData.rows.map(mapArticleSummaryRow);

    var normalizedSearch = toSafeString(f.searchText).toLowerCase();

    var results = summaries.filter(function (a) {
      if (f.category && normalizeHeader(a.category) !== normalizeHeader(f.category)) { return false; }
      if (f.status && normalizeHeader(a.status) !== normalizeHeader(f.status)) { return false; }
      if (f.departmentId && a.departmentIds.indexOf(toSafeString(f.departmentId)) === -1) { return false; }

      if (normalizedSearch) {
        var haystack = [a.title, a.category, a.tags.join(' '), a.criteria.join(' ')].join(' ').toLowerCase();
        if (haystack.indexOf(normalizedSearch) === -1) { return false; }
      }
    ...
    results.sort(function (a, b) {
      var dateA = parseDate(a.updatedDate);
      var dateB = parseDate(b.updatedDate);
  ...
  // Full article detail, including body sections - only fetched when an
  // article is actually opened, not as part of any list view.
  function getArticleById(articleId) {
    var requestedId = toSafeString(articleId);
    if (!requestedId) {
      return { success: false, message: 'An Article ID is required.' };
    }

    var articleData = readSheet(SHEETS.knowledgeBaseArticles);
    var row = findRowByField(articleData.rows, FIELD_NAMES.articleId, requestedId);
    if (!row) {
      return { success: false, message: 'Article ' + requestedId + ' could not be found.' };
    }

    var departmentData = readSheet(SHEETS.departments);
    var departmentNameById = {};
    departmentData.rows.forEach(function (d) {
      departmentNameById[toSafeString(getFieldValue(d, [FIELD_NAMES.departmentId]))] = toSafeString(getFieldValue(d, [FIELD_NAMES.departmentName]));
    });

    var departmentIds = parseCommaList(getFieldValue(row, [FIELD_NAMES.articleDepartmentIds]));
    var relatedIds = parseCommaList(getFieldValue(row, [FIELD_NAMES.articleRelatedIds]));
    var relatedArticles = relatedIds.map(function (id) {
      var relatedRow = findRowByField(articleData.rows, FIELD_NAMES.articleId, id);
      return relatedRow ? { articleId: id, title: toSafeString(getFieldValue(relatedRow, [FIELD_NAMES.articleTitle])) } : null;
    }).filter(Boolean);
    ...
    return {
      ...
      article: {
        ...
        departmentNames: departmentIds.map(function (id) { return departmentNameById[id] || id; }),
        templateIds: parseCommaList(getFieldValue(row, [FIELD_NAMES.articleTemplateIds])),
        criteria: parseCommaList(getFieldValue(row, [FIELD_NAMES.articleCriteria])),
        tags: parseCommaList(getFieldValue(row, [FIELD_NAMES.articleTags])),
        status: toSafeString(getFieldValue(row, [FIELD_NAMES.articleStatus])),
        version: toSafeString(getFieldValue(row, [FIELD_NAMES.articleVersion])),
        author: toSafeString(getFieldValue(row, [FIELD_NAMES.articleAuthor])),
        createdDate: serializeValue(getFieldValue(row, [FIELD_NAMES.articleCreatedDate])),
        updatedDate: serializeValue(getFieldValue(row, [FIELD_NAMES.articleUpdatedDate])),
        purpose: toSafeString(getFieldValue(row, [FIELD_NAMES.articlePurpose])),
        policy: toSafeString(getFieldValue(row, [FIELD_NAMES.articlePolicy])),
        procedure: toSafeString(getFieldValue(row, [FIELD_NAMES.articleProcedure])),
        examples: toSafeString(getFieldValue(row, [FIELD_NAMES.articleExamples])),
        commonMistakes: toSafeString(getFieldValue(row, [FIELD_NAMES.articleCommonMistakes])),
        coachingNotes: toSafeString(getFieldValue(row, [FIELD_NAMES.articleCoachingNotes])),
        relatedArticles: relatedArticles
      }
    };
  }

  // Finds every Published article that references a given criterion name -
  // the foundation the (later) Criterion Integration phase will call from
  // Audit Details, Agent Profile, Reports, Search, and Root Cause drill-
  // down. Built now, alongside the data model, since every one of those
  // call sites will need exactly this lookup.
  function getArticlesForCriterion(criterionName) {
    var normalized = normalizeHeader(criterionName);
    if (!normalized) { return { success: true, articles: [] }; }

    var articleData = readSheet(SHEETS.knowledgeBaseArticles);
    var matches = articleData.rows
      .map(mapArticleSummaryRow)
      .filter(function (a) {
        return normalizeHeader(a.status) === 'published' &&
          a.criteria.some(function (c) { return normalizeHeader(c) === normalized; });
      });

    return { success: true, articles: matches };
  }
  ...
  function saveArticle(payload) {
    ...
    if (!articleData.headers.length) {
      throw new Error(SHEETS.knowledgeBaseArticles + ' must contain a header row before data can be saved.');
    }
    if (!toSafeString(getPayloadValue(values, FIELD_NAMES.articleTitle))) {
      throw new Error('Title is required.');
    }

    var articleId = generateNextSequentialId(articleData.rows, [FIELD_NAMES.articleId], 'KB-', 4);
    var today = new Date();

    var row = articleData.headers.map(function (header) {
      var normalized = normalizeHeader(header);
      if (normalized === normalizeHeader(FIELD_NAMES.articleId)) { return articleId; }
      if (normalized === normalizeHeader(FIELD_NAMES.articleVersion)) { return '1'; }
      if (normalized === normalizeHeader(FIELD_NAMES.articleCreatedDate)) { return coerceValueForSheet(header, today); }
      if (normalized === normalizeHeader(FIELD_NAMES.articleUpdatedDate)) { return coerceValueForSheet(header, today); }
      if (normalized === normalizeHeader(FIELD_NAMES.articleStatus) && isEmptyValue(getPayloadValue(values, header))) { return 'Draft'; }
      return coerceValueForSheet(header, getPayloadValue(values, header));
    });

    articleData.sheet.appendRow(row);

    return { success: true, message: 'Article saved successfully (' + articleId + ').', articleId: articleId };
  }

  // Edits an article in place (Phase A: no version-branch workflow yet -
  // see the module comment above). Updated Date is always stamped to
  // today, regardless of what else changed.
  function updateArticle(articleId, payload) {
    var requestedId = toSafeString(articleId);
    if (!requestedId) {
      throw new Error('An Article ID is required to update an article.');
    }

    var values = {};
    var payloadValues = (payload && payload.values) || {};
    Object.keys(payloadValues).forEach(function (key) { values[key] = payloadValues[key]; });
    values[FIELD_NAMES.articleUpdatedDate] = new Date();

    updateRowById(SHEETS.knowledgeBaseArticles, FIELD_NAMES.articleId, requestedId, values);

    return { success: true, message: 'Article ' + requestedId + ' updated successfully.' };
  }

  function setArticleStatus(articleId, status) {
    var requestedId = toSafeString(articleId);
    if (!requestedId) {
      throw new Error('An Article ID is required.');
    }

    var values = {};
    values[FIELD_NAMES.articleStatus] = status;
    values[FIELD_NAMES.articleUpdatedDate] = new Date();
    updateRowById(SHEETS.knowledgeBaseArticles, FIELD_NAMES.articleId, requestedId, values);

    return { success: true, message: 'Article ' + requestedId + ' marked as ' + status + '.' };
  }

  // =========================================================================
  // COACHING MODULE
  // Extends the existing Coaching Log sheet in place (per explicit decision:
  // no new sheet) rather than duplicating it with a parallel structure.
  // saveCoaching() and saveLogEntry() are untouched - every new flat column
  // added below is already handled generically by that existing header-
  // driven writer. Only two genuinely new capabilities were needed:
  // updateCoaching() (Coaching Log never supported edits before) and Action
  // Plan parsing (the one field that's inherently one-to-many).
  // =========================================================================

  // Action Plan is stored as a JSON array in a single cell - the one field
  // here that's genuinely one-to-many and can't be a flat column. This is
  // the one place in the whole sheet that isn't hand-edit-friendly; every
  // other new column is a plain value like anywhere else in the app.
  function parseActionPlan(rawValue) {
    var raw = toSafeString(rawValue);
    if (!raw) { return []; }
    try {
  ...
  function serializeActionPlan(actionItems) {
    return JSON.stringify(Array.isArray(actionItems) ? actionItems : []);
  }

  function isCoachingOverdue(row) {
    var status = toSafeString(getFieldValue(row, [FIELD_NAMES.coachingStatus]));
    if (['completed', 'cancelled'].indexOf(normalizeHeader(status)) !== -1) { return false; }

    var today = new Date();
    var followUpDate = parseDate(getFieldValue(row, [FIELD_NAMES.followUpDate]));
    var dueDate = parseDate(getFieldValue(row, [FIELD_NAMES.coachingDueDate]));

    return (followUpDate && followUpDate.getTime() < today.getTime()) ||
      (dueDate && dueDate.getTime() < today.getTime());
  }

  function mapCoachingSummaryRow(row, agentInfoById) {
    var agentId = toSafeString(getFieldValue(row, [FIELD_NAMES.agentId]));
    var agentInfo = agentInfoById[agentId] || {};

    return {
      coachingId: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingId])),
      agentId: agentId,
      agentName: agentInfo.name || '',
      departmentId: agentInfo.departmentId || '',
      departmentName: agentInfo.departmentName || '',
      teamLeader: agentInfo.teamLeader || '',
      coach: toSafeString(getFieldValue(row, [FIELD_NAMES.coach])),
      auditId: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingAuditId])),
      sessionDate: serializeValue(getFieldValue(row, [FIELD_NAMES.date])),
      coachingType: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingType])),
      status: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingStatus])),
      priority: toSafeString(getFieldValue(row, [FIELD_NAMES.priority])),
      goal: toSafeString(getFieldValue(row, [FIELD_NAMES.goal])),
      followUpDate: serializeValue(getFieldValue(row, [FIELD_NAMES.followUpDate])),
      dueDate: serializeValue(getFieldValue(row, [FIELD_NAMES.coachingDueDate])),
      templateId: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingTemplateId])),
      isOverdue: isCoachingOverdue(row)
    };
  }

  function buildAgentInfoLookup() {
    var departmentData = readSheet(SHEETS.departments);
    var departmentNameById = {};
    departmentData.rows.forEach(function (d) {
      departmentNameById[toSafeString(getFieldValue(d, [FIELD_NAMES.departmentId]))] = toSafeString(getFieldValue(d, [FIELD_NAMES.departmentName]));
  ...
  // List/search coaching sessions. filters: { departmentId, teamLeader,
  // coach, agentId, status, priority, dateFrom, dateTo, templateId,
  // searchText, limit, offset } - limit/offset support simple pagination.
  function getCoachingSessions(filters) {
    var f = filters || {};
    var coachingData = readSheet(SHEETS.coachingLog);
    var agentInfoById = buildAgentInfoLookup();

    var summaries = coachingData.rows.map(function (row) { return mapCoachingSummaryRow(row, agentInfoById); });
    ...
    var filtered = summaries.filter(function (s) {
      if (f.departmentId && s.departmentId !== toSafeString(f.departmentId)) { return false; }
      if (f.teamLeader && normalizeHeader(s.teamLeader) !== normalizeHeader(f.teamLeader)) { return false; }
      if (f.coach && normalizeHeader(s.coach) !== normalizeHeader(f.coach)) { return false; }
      if (f.agentId && s.agentId !== toSafeString(f.agentId)) { return false; }
      if (f.status && normalizeHeader(s.status) !== normalizeHeader(f.status)) { return false; }
      if (f.priority && normalizeHeader(s.priority) !== normalizeHeader(f.priority)) { return false; }
      if (f.templateId && s.templateId !== toSafeString(f.templateId)) { return false; }

      if (dateFrom || dateTo) {
        var sessionDate = parseDate(s.sessionDate);
        if (!sessionDate) { return false; }
        if (dateFrom && sessionDate.getTime() < dateFrom.getTime()) { return false; }
        if (dateTo && sessionDate.getTime() > dateTo.getTime()) { return false; }
      }

      if (normalizedSearch) {
        var haystack = [s.agentName, s.coach, s.goal, s.auditId].join(' ').toLowerCase();
        if (haystack.indexOf(normalizedSearch) === -1) { return false; }
      }
    ...
    filtered.sort(function (a, b) {
      var dateA = parseDate(a.sessionDate);
      var dateB = parseDate(b.sessionDate);
      return (dateB ? dateB.getTime() : 0) - (dateA ? dateA.getTime() : 0);
    });

    var totalCount = filtered.length;
    var offset = toNumber(f.offset) || 0;
    var limit = toNumber(f.limit) || totalCount;
    var page = filtered.slice(offset, offset + limit);

    return { success: true, totalCount: totalCount, sessions: page };
  }

  // Full detail for one coaching session: parsed Action Plan, the linked
  // audit's detail (reusing buildAuditAnalyticsDataset + matchesAuditId -
  // no re-derived join), the agent's recent performance timeline (reusing
  // buildPerformanceTimeline), and a simple chronological Status Timeline
  // built from this record's own date fields.
  function getCoachingSessionById(coachingId) {
    var requestedId = toSafeString(coachingId);
    if (!requestedId) {
      return { success: false, message: 'A Coaching ID is required.' };
    }

    var coachingData = readSheet(SHEETS.coachingLog);
    var row = findRowByField(coachingData.rows, FIELD_NAMES.coachingId, requestedId);
    if (!row) {
      return { success: false, message: 'Coaching session ' + requestedId + ' could not be found.' };
    }

    var agentInfoById = buildAgentInfoLookup();
    var summary = mapCoachingSummaryRow(row, agentInfoById);

    var linkedAudit = null;
    if (summary.auditId) {
      var dataset = buildAuditAnalyticsDataset();
      linkedAudit = dataset.find(function (r) { return matchesAuditId(r, summary.auditId); }) || null;
    }

    var performanceData = readSheet(SHEETS.performanceLog);
    var agentPerformanceRows = filterRowsByAgentId(performanceData.rows, summary.agentId);
    var recentPerformance = buildPerformanceTimeline(agentPerformanceRows).weekly.slice(-8);

    var statusTimeline = buildCoachingStatusTimeline(row);

    return {
      success: true,
      session: {
        coachingId: summary.coachingId,
        agentId: summary.agentId,
        agentName: summary.agentName,
        departmentName: summary.departmentName,
        teamLeader: summary.teamLeader,
        coach: summary.coach,
        sessionDate: summary.sessionDate,
        coachingType: summary.coachingType,
        status: summary.status,
        priority: summary.priority,
        goal: summary.goal,
        summary: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingSummary])),
        strengths: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingStrengths])),
        developmentAreas: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingDevelopmentAreas])),
        actionPlan: parseActionPlan(getFieldValue(row, [FIELD_NAMES.actionPlan])),
        successMeasure: toSafeString(getFieldValue(row, [FIELD_NAMES.successMeasure])),
        dueDate: summary.dueDate,
        followUpDate: summary.followUpDate,
        reAuditDate: serializeValue(getFieldValue(row, [FIELD_NAMES.reAuditDate])),
        completionDate: serializeValue(getFieldValue(row, [FIELD_NAMES.completionDate])),
        managerNotes: toSafeString(getFieldValue(row, [FIELD_NAMES.managerNotes])),
        employeeComments: toSafeString(getFieldValue(row, [FIELD_NAMES.employeeComments])),
        nextReview: serializeValue(getFieldValue(row, [FIELD_NAMES.nextReview])),
  ...
  function buildCoachingStatusTimeline(row) {
    var events = [
      { label: 'Audit Completed', date: getFieldValue(row, [FIELD_NAMES.coachingAuditDate]) },
      { label: 'Coaching Scheduled / Held', date: getFieldValue(row, [FIELD_NAMES.date]) },
      { label: 'Follow-up', date: getFieldValue(row, [FIELD_NAMES.followUpDate]) },
      { label: 'Re-audit', date: getFieldValue(row, [FIELD_NAMES.reAuditDate]) },
      { label: 'Completed', date: getFieldValue(row, [FIELD_NAMES.completionDate]) }
    ];

    return events
      .map(function (e) { return { label: e.label, date: parseDate(e.date) }; })
      .filter(function (e) { return e.date; })
      .sort(function (a, b) { return a.date.getTime() - b.date.getTime(); })
      .map(function (e) { return { label: e.label, date: formatDate(e.date) }; });
  }

  // For "Create Coaching from Audit" - returns everything needed to
  // pre-fill the form (Agent, Department, Template, Score, Failed Criteria,
  // Comments) WITHOUT storing any of it redundantly on the coaching record
  // itself; only the Audit ID is stored, and this same lookup re-derives
  // the rest at display time (see getCoachingSessionById's linkedAudit).
  function getCoachingContextFromAudit(auditId) {
    var requestedId = toSafeString(auditId);
    if (!requestedId) {
      return { success: false, message: 'An Audit ID is required.' };
    }

    var dataset = buildAuditAnalyticsDataset();
    var record = dataset.find(function (r) { return matchesAuditId(r, requestedId); });
    if (!record) {
      return { success: false, message: 'No audit found matching ID "' + requestedId + '".' };
    }
  ...
  function saveCoachingActionPlan(coachingId, actionItems) {
    var values = {};
    values[FIELD_NAMES.actionPlan] = serializeActionPlan(actionItems);
    values[FIELD_NAMES.coachingUpdatedDate] = new Date();
    updateRowById(SHEETS.coachingLog, FIELD_NAMES.coachingId, toSafeString(coachingId), values);
    return { success: true, message: 'Action plan updated.' };
  }

  // First edit capability Coaching Log has ever had - reuses the same
  // updateRowById() helper as Scorecard Templates/Criteria/KB Articles.
  // Updated Date is always stamped, regardless of what else changed.
  function updateCoaching(coachingId, payload) {
    var requestedId = toSafeString(coachingId);
    if (!requestedId) {
      throw new Error('A Coaching ID is required to update a coaching session.');
    }
    ...
    updateRowById(SHEETS.coachingLog, FIELD_NAMES.coachingId, requestedId, values);
    return { success: true, message: 'Coaching session ' + requestedId + ' updated successfully.' };
  }

  // Dashboard + reusable widget data. "Prepare hooks for future reminders"
  // (no email) is satisfied by upcomingFollowUps/overdueSessions/
  // upcomingReAudits below - a future time-driven trigger would consume
  // exactly these lists, nothing more needs to be built for that yet.
  function getCoachingDashboard() {
    ...
    var today = new Date();
    var in14Days = new Date(today.getTime() + 14 * 86400000);
    var monthKey = today.getFullYear() + '-' + padNumber(today.getMonth() + 1, 2);

    var openSessions = summaries.filter(function (s) {
      return ['completed', 'cancelled'].indexOf(normalizeHeader(s.status)) === -1;
    });

    var completedThisMonth = summaries.filter(function (s) {
      return normalizeHeader(s.status) === 'completed' && getMonthPeriodKey(parseDate(s.sessionDate) || today) === monthKey;
    });

    var overdueSessions = summaries.filter(function (s) { return s.isOverdue; });

    var upcomingFollowUps = summaries.filter(function (s) {
      var followUp = parseDate(s.followUpDate);
      return followUp && followUp.getTime() >= today.getTime() && followUp.getTime() <= in14Days.getTime() &&
        ['completed', 'cancelled'].indexOf(normalizeHeader(s.status)) === -1;
    }).sort(function (a, b) { return parseDate(a.followUpDate).getTime() - parseDate(b.followUpDate).getTime(); });

    var upcomingReAudits = coachingData.rows
      .map(function (row) { return { coachingId: toSafeString(getFieldValue(row, [FIELD_NAMES.coachingId])), reAuditDate: serializeValue(getFieldValue(row, [FIELD_NAMES.reAuditDate])) }; })
      .filter(function (s) {
        var reAuditDate = parseDate(s.reAuditDate);
        return reAuditDate && reAuditDate.getTime() >= today.getTime() && reAuditDate.getTime() <= in14Days.getTime();
      });

    var agentsCurrentlyBeingCoached = distinctNonEmpty(openSessions.map(function (s) { return s.agentId; }));

    var averageImprovement = computeAverageImprovementAfterCoaching(summaries);

    var mostCommonDevelopmentAreas = aggregateFailedCriteriaFromRecords(
      buildAuditAnalyticsDataset().filter(function (r) { return agentsCurrentlyBeingCoached.indexOf(r.agentId) !== -1; })
    ).slice(0, 5);
    ...
    return {
      ...
      upcomingReAudits: upcomingReAudits,
      agentsCurrentlyCoachedCount: agentsCurrentlyBeingCoached.length,
      averageImprovementAfterCoaching: averageImprovement,
      mostCommonDevelopmentAreas: mostCommonDevelopmentAreas,
      completionRate: summaries.length
        ? roundNumber((summaries.filter(function (s) { return normalizeHeader(s.status) === 'completed'; }).length /
            summaries.filter(function (s) { return normalizeHeader(s.status) !== 'cancelled'; }).length) * 100, 1)
        : null
    };
  }

  // For every Completed session linked to an audit, compares the agent's
  // average score in the 4 weeks before the session to the 4 weeks after.
  // Averaged across all such sessions. Documented, not a guaranteed causal
  // measure - other factors can move an agent's score in that window too.
  function computeAverageImprovementAfterCoaching(summaries) {
    var completedWithDate = summaries.filter(function (s) {
      return normalizeHeader(s.status) === 'completed' && parseDate(s.sessionDate);
    });
    if (!completedWithDate.length) { return null; }

    var dataset = buildAuditAnalyticsDataset();
    var deltas = [];

    completedWithDate.forEach(function (session) {
      var sessionDate = parseDate(session.sessionDate);
      var windowStart = new Date(sessionDate.getTime() - 28 * 86400000);
      var windowEnd = new Date(sessionDate.getTime() + 28 * 86400000);

      var before = dataset.filter(function (r) {
        return r.agentId === session.agentId && r.auditDateObj &&
          r.auditDateObj.getTime() >= windowStart.getTime() && r.auditDateObj.getTime() < sessionDate.getTime();
      });
      var after = dataset.filter(function (r) {
        return r.agentId === session.agentId && r.auditDateObj &&
          r.auditDateObj.getTime() > sessionDate.getTime() && r.auditDateObj.getTime() <= windowEnd.getTime();
      });

      var beforeAvg = computeAverageScore(before);
      var afterAvg = computeAverageScore(after);
      if (typeof beforeAvg === 'number' && typeof afterAvg === 'number') {
  ...
  return {
    ...
    getKnowledgeBaseHomepage: getKnowledgeBaseHomepage,
    getKnowledgeBaseArticles: getKnowledgeBaseArticles,
    getArticleById: getArticleById,
    getArticlesForCriterion: getArticlesForCriterion,
    saveArticle: saveArticle,
    updateArticle: updateArticle,
    setArticleStatus: setArticleStatus,
    getCoachingSessions: getCoachingSessions,
    getCoachingSessionById: getCoachingSessionById,
    getCoachingContextFromAudit: getCoachingContextFromAudit,
